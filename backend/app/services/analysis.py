...
def _analyze_with_sliding_window(
    ...
    patches = extract_patches(
        ...
        stride=IMAGE_SIZE // 2,  # 50% overlap
    )
    
    logger.info(f"Analyzing {len(patches)} patches")
    
    nodules = []
    threshold = 0.4  # Minimum confidence to consider a detection
...
def _remove_overlapping_nodules(nodules: list[Nodule], overlap_threshold: float = 0.5) -> list[Nodule]:
    ...
    for nodule in sorted_nodules:
        # Check overlap with existing nodules
        overlaps = False
        for existing in filtered:
            distance = (
                (nodule.x - existing.x) ** 2 + (nodule.y - existing.y) ** 2
            ) ** 0.5
            min_radius = min(nodule.radius, existing.radius)
            max_radius = max(nodule.radius, existing.radius)
            
            # If distance is less than overlap_threshold of the larger radius, consider overlapping
            if distance < max_radius * overlap_threshold:
                overlaps = True
                break
