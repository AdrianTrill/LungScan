...
# ==========================================
# 1. WORKER FUNCTION (Parallel Execution)
# ==========================================
def process_patient(args_tuple):
    ...
    try:
        ...
        # --- LOAD PREDICTION ---
        if is_npz:
            # Load Probabilities (For custom thresholding)
            npz_content = np.load(pred_path)
            probs = npz_content['probabilities'] # Shape is usually (Channels, Z, Y, X)
            
            # Select Tumor Channel (Index 1)
            channel_idx = 1 if probs.shape[0] > 1 else 0
            tumor_prob = probs[channel_idx]
            
            # --- CRITICAL ORIENTATION FIX ---
            # nnU-Net saves .npz as (Z, Y, X). Nibabel loads NIfTI as (X, Y, Z).
            # We MUST transpose the probability map to match the Ground Truth.
        ...
        if pred_data.shape != gt_data.shape:
            # If shapes still don't match, we skip to avoid crashing
            return None 

        # --- CLEAN PREDICTIONS (Remove Small Blobs) ---
        # Dynamic threshold based on voxel size (mm -> voxels)
        ...
        # If healthy and we predicted nothing -> Perfect score
        ...
        # Check Ground Truths (Did we find them?)
        if n_gt > 0:
            for i in range(1, n_gt + 1):
                ...
                if len(overlap_ids) > 0:
                    ...
                    # Calculate Dice for this specific lesion overlap
        ...
        # Check Predictions (Are they FPs?)
        if n_pred > 0:
...
# ==========================================
def main():
    parser = argparse.ArgumentParser(description="Unified Evaluation Script with Multiprocessing")
    ...
    if args.force_predict:
        ...
        print(f">>> Found existing files. Skipping Inference.")
    ...
    if run_inference:
        ...
        # Always save probabilities just in case
    ...
    # This divides the file list among 8 workers (or whatever you specify)
    with multiprocessing.Pool(args.workers) as pool:
        # imap combined with tqdm gives a nice progress bar
