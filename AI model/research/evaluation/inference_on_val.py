...
#     """
#     Scans dataset for a 'Small' nodule.
#     Small is defined as 10 to 300 voxels (approx 4mm - 12mm).
#     """
#     print(f"Searching for a SMALL nodule (10-300 voxels) in {data_root}...")
#     label_files = sorted(glob.glob(os.path.join(data_root, "labels", "*.nii.gz")))
    
#     # Check up to 100 files to find a good candidate
#     for lbl_path in label_files[:200]: 
#         lbl_vol = nib.load(lbl_path).get_fdata()
#         voxel_count = np.sum(lbl_vol)
        
#         # Criteria for "Small": Not empty, not huge
#         if 10 < voxel_count < 300:
...
#                 print(f"Found Candidate: {img_filename}")
#                 print(f" -> Nodule Volume: {voxel_count:.0f} voxels")
#                 return img_path, lbl_path
                
#     print("Warning: Could not find a 'Small' nodule in the first 200 files. Using the first positive one found.")
#     # Fallback to any positive sample
#     for lbl_path in label_files:
#         if np.sum(nib.load(lbl_path).get_fdata()) > 0:
#             img_filename = os.path.basename(lbl_path).replace("_label.nii.gz", "_image.nii.gz")
#             return os.path.join(data_root, "images", img_filename), lbl_path
...
def main():
    ...
    plt.tight_layout()
    output_png = "vista_small_nodule_our_checkpoint_4.png"
